...
label1:
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutstate, RecyclerView.State state)
    {
        ...
        if (layoutstate.mLayoutDirection == 1)
        {
            j = layoutstate.mEndLine + layoutstate.mAvailable;
        } else
        {
            j = layoutstate.mStartLine - layoutstate.mAvailable;
        }
        updateAllRemainingSpans(layoutstate.mLayoutDirection, j);
        if (mShouldReverseLayout)
        {
        ...
        do
        {
            if (layoutstate.hasMore(state) && !mRemainingSpans.isEmpty())
            {
                ...
                if (layoutstate.mLayoutDirection == 1)
                {
                    ...
                    k1 = i + mPrimaryOrientation.getDecoratedMeasurement(view);
                    l = i;
                    i1 = k1;
                    if (j1 != 0)
                    {
                ...
                } else
                {
                    ...
                    {
                        i = span.getStartLine(k);
                    }
                    l1 = i - mPrimaryOrientation.getDecoratedMeasurement(view);
                    l = l1;
                    i1 = i;
                    if (j1 != 0)
                    {
                ...
                j1 = i + mSecondaryOrientation.getDecoratedMeasurement(view);
                if (mOrientation == 1)
                {
                    layoutDecoratedWithMargins(view, i, l, j1, i1);
                } else
                {
                    layoutDecoratedWithMargins(view, l, i, i1, j1);
                }
                if (layoutparams.mFullSpan)
                {
                ...
                {
                    updateRemainingSpans(span, mLayoutState.mLayoutDirection, j);
                }
...
_L1:
        if (mGapStrategy == 0 || getChildCount() <= 0) goto _L4; else goto _L3
_L3:
        flag = flag1;
        if (mLaidOutInvalidFullSpan) goto _L6; else goto _L5
_L5:
        if (hasGapsToFix() == null) goto _L4; else goto _L7
_L7:
        flag = flag1;
_L6:
        if (flag)
